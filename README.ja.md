...
毎朝のアラームを「アファメーション（肯定的なフレーズ）」の読み上げに置き換えるクロスプラットフォームモバイルアプリ（iOS / Android）。設定した時刻に好きなボイスでフレーズを読み上げ、ポジティブな意図を朝のルーチンに組み込みます。

**Flutter** で実装予定。認証は Firebase Authentication、フレーズ生成は BYOK（Bring Your Own Key）方式の Claude API 連携を採用。

> **ステータス: 計画中 / 開発前。** リリース未公開。設計書は [HANDOFF_affirmation-alarm.md](HANDOFF_affirmation-alarm.md) を参照。
...
|---|---|
| アプリ | Flutter (Dart) |
| ローカルDB | SQLite または Hive |
| 認証 | Firebase Authentication |
| AI（オプション） | Claude API — BYOK のみ |
| バックエンド | Hono on Fly.io（Firebaseトークン検証のみ） |
| TTS | `AVSpeechSynthesizer`（iOS） / `TextToSpeech`（Android） |
| 通知 | `flutter_local_notifications` + 各OSのアラームAPI |

Claude API は Keychain / Keystore に保存したユーザー自身のAPIキーを使って **アプリから直接** 呼び出します。AIリクエスト用のプロキシサーバーは経由しません。
...
現時点では設計書のみを含みます。Flutter プロジェクトをスキャフォールドした時点でソースコードを追加します。
